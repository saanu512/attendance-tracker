...
function attendanceAdvice(r){
  ...
  if(q>=1){
    ...
    return '<span class="adviceRed">⚠ Need to attend all future classes; 100% can only be reached if no class was missed</span>';
  }
  if(r.pct>=req){
    ...
    return '<span class="adviceGreen">✓ Attendance is enough</span><span class="adviceRed"> — you can miss '+canMiss+' more class'+(canMiss===1?'':'es')+' and still stay at or above '+req+'%</span>';
  }
...
load();applyTheme();watchSystemTheme();render();if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",activateGate,{once:true});}else{activateGate();}if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=40-roshni-pearl-update").then(r=>r.update?.()).catch(()=>{}));
