...
function rememberCloudSync(uid,date=new Date()){if(!uid||!(date instanceof Date)||Number.isNaN(date.getTime()))return;localStorage.setItem(cloudLastSyncKey(uid),date.toISOString());cloudSession.lastSync=date;}
function restoreCloudSync(uid,fallback){if(!uid)return null;let raw=localStorage.getItem(cloudLastSyncKey(uid));let d=raw?new Date(raw):null;if(!(d instanceof Date)||Number.isNaN(d?.getTime?.()))d=null;if(!d&&fallback){d=new Date(fallback);if(Number.isNaN(d.getTime()))d=null;}cloudSession.lastSync=d;return d;}
function cloudSyncState(){if(!cloudSession.user)return {label:"Not signed in",cls:""};if(cloudSession.error)return {label:"● Sync failed",cls:"danger"};if(cloudSession.syncing)return {label:"● Syncing…",cls:"safe"};return {label:"● Cloud sync working",cls:"safe"};}
...
function updateCloudStatus(){const el=document.getElementById("cloudStatusText");if(!el)return;const st=cloudSyncState();el.innerHTML=`<span class="cloudSyncState ${st.cls}">${st.label}</span><span class="cloudLastSync">${esc(cloudLastSyncLabel())}</span>`;el.className="small cloudStatusBlock";}
...
function adminStudentCards(students){return students.map(s=>`<button type="button" class="adminStudent card" data-act="adminStudent" data-uid="${esc(s.uid)}"><div class="adminStudentMain"><b>${esc(s.name||"Student")}</b><span>Roll: ${esc(s.rollNumber||"—")}</span></div><div class="small">${esc(s.email||"")}</div><div class="adminStudentMeta"><span>Required ${Number(s.required||75)}%</span><span>${esc(s.course||"")}</span><span class="adminSyncStamp">${esc(adminSyncLabel(s))}</span></div></button>`).join('')||`<div class="card formCard"><p class="small">No matching students.</p></div>`}
