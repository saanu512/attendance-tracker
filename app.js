...
function dailyInfoData(dk){
  const d=parseKey(dk),base=(state.settings.schedule[d.getDay()]||[]).map(x=>clone(x)),changes=[];
  ...
  Object.keys(ov).forEach(k=>{
    ...
    if(original[0]===actual[0]&&original[1]===actual[1]&&original[2]===actual[2])return;
    changes.push({originalSubject:original[2],originalStart:original[0],originalEnd:original[1],actualSubject:actual[2],actualStart:actual[0],actualEnd:actual[1],type:original[2]===actual[2]?'rescheduled':'replaced'});
  });
...
function infoIndicator(dk){const info=dailyInfoData(dk);if(!info.hasSchedule&&!info.hasNote)return '';const tone=info.hasSchedule&&info.hasNote?'both':info.hasSchedule?'schedule':'note',icon=info.hasSchedule&&info.hasNote?'📝🔄':info.hasSchedule?'🔄':'📝',label=info.hasSchedule&&info.hasNote?'Notes and class changes':info.hasSchedule?'Class changes and extra classes':'Saved note';return `<div class="infoIndicatorRow"><button type="button" class="dailyInfoIndicator ${tone}" data-act="dailyInfo" aria-label="${label}" title="${label}">${icon}</button></div>`}
function dailyInfoPage(){const dk=state.viewDate,d=parseKey(dk),info=dailyInfoData(dk),note=String(state.notes&&state.notes[dk]||'').trim();let h=`<div class="dailyInfoPage"><button type="button" class="btn outline dailyInfoBack" data-act="backFromInfo">‹ Back to Log</button><div class="card formCard dailyInfoHeader"><div class="dailyInfoDate">${DAYS[d.getDay()]}</div><div class="dailyInfoDateSub">${fmt(d)}</div></div>`;if(info.hasSchedule){h+=`<div class="card formCard dailyInfoCard"><h3>🔄 Class Changes / Schedule Changes</h3>`;info.changes.forEach(c=>{if(c.type==='rescheduled')h+=`<div class="infoItem"><b>${esc(c.originalSubject)}</b><div>${time(c.originalStart)} – ${time(c.originalEnd)} → ${time(c.actualStart)} – ${time(c.actualEnd)}</div><span>Rescheduled</span></div>`;else h+=`<div class="infoItem"><b>Originally: ${esc(c.originalSubject)}</b><div>${time(c.originalStart)} – ${time(c.originalEnd)}</div><div><b>Actually held: ${esc(c.actualSubject)}</b> · ${time(c.actualStart)} – ${time(c.actualEnd)}</div><span>Replacement</span></div>`});info.extras.forEach(x=>h+=`<div class="infoItem extraInfoItem"><b>➕ Extra class: ${esc(x.subject)}</b><div>${time(x.start)} – ${time(x.end)}</div></div>`);h+=`</div>`}if(info.hasNote)h+=`<div class="card formCard dailyInfoCard savedNotesCard"><h3>📝 Saved Note</h3><div class="savedNoteText">${esc(note).replace(/\n/g,'<br>')}</div></div>`;return h+`</div>`}
function logTab(){let d=parseKey(state.viewDate),dk=key(d),ss=sessionsFor(d),past=d<today(),future=d>today(),can=editable(dk),before=!past&&!future&&!can,isHoliday=!!state.holidays[dk],p=dayProgress(d);let h=dateControls();h+=infoIndicator(dk);if(d<START_DATE)return h+`<div class="banner lock">🔒 Attendance tracking starts on 7 Sep 2026.</div>`;if(ss.length)h+=`<div class="card progressCard"><div class="progressHead"><b>Today's progress</b><span class="progressText">${isHoliday?"Holiday":p.marked+" / "+p.total+" marked"}</span></div><div class="progress"><i style="width:${p.total?p.marked/p.total*100:0}%"></i></div></div>`;if(before)h+=`<div class="banner warn">🕕 Logging opens at 6:00 AM.</div>`;else if(past)h+=`<div class="banner info">🕘 Past date: classes not yet saved can be entered once. Already saved classes are permanently locked.</div>`;else if(future)h+=`<div class="banner lock">🔒 Future date — it becomes editable when that day arrives.</div>`;else h+=`<div class="banner info">✓ Today is editable until midnight.</div>`;if(!ss.length)return h+`<div class="card formCard" style="text-align:center;color:var(--soft)"><div style="font-weight:850;font-size:17px">No classes scheduled</div><div style="margin-top:6px">You can add an extra class for this date.</div></div>${can?`<div class="attendanceActions"><button class="saveAttendance actionCompact" data-act="addExtra">＋ Add extra class</button></div>`:`<div class="card formCard" style="text-align:center;color:var(--soft)">🔒 Extra classes can be added when this date becomes editable.</div>`}`;h+=`<button class="holiday" data-act="holiday" ${can?"":"disabled"}>${isHoliday?"🏖 Marked as holiday"+(can?" — tap to undo":""):can?"🏖 Mark whole day as holiday":"🔒 Not a holiday"}</button>`;if(!isHoliday){ss.forEach((s,i)=>{let cur=draftValue(sessionKey(dk,i)),classCan=attendanceEditable(dk,i),locked=classCan?"":"disabled",changed=s._scheduled&&s._scheduled!==s[2];h+=`<div class="card session"><div class="sessionTop"><div><div class="sessionName">${esc(s[2])}${s[3]?" 📌":""}${changed?" 🔄":""}</div>${changed?`<div class="small">Scheduled: ${esc(s._scheduled)}</div>`:""}<div class="sessionTime">${time(s[0])} – ${time(s[1])}</div></div><div>${classCan?`${s[3]?`<div style="display:flex;gap:6px;justify-content:flex-end"><button class="btn outline" style="padding:7px 9px;font-size:11px" data-act="editClass" data-i="${i}">✏️ Edit</button></div>`:`<button class="btn outline" style="padding:7px 9px;font-size:11px" data-act="editClass" data-i="${i}">✏️ Edit</button>`}`:`<div class="locktag">${past?"🔒 Saved & locked":future?"🔒 Future":"🕕 Opens later"}</div>`}</div></div><div class="statusRow"><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="attended" class="status green ${cur==="attended"?"active":""}">✓<br>Present</button><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="absent" class="status red ${cur==="absent"?"active":""}">✕<br>Absent</button><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="not_held" class="status gray ${cur==="not_held"?"active":""}">⊘<br>Not held</button></div></div>`});if(can){h+=`<div class="attendanceActions"><button class="saveAttendance actionCompact" data-act="addExtra">＋ Add extra class</button><button class="saveAttendance actionCompact" data-act="saveAttendance">💾 Save attendance</button></div>`}h+=`<div class="card noteBox"><h3>📝 Note for this day</h3><textarea id="dayNote" ${can?"":"disabled"} placeholder="Optional note...">${esc(state.notes[dk]||"")}</textarea>${can?`<div style="margin-top:8px"><button class="saveAttendance actionCompact" data-act="saveNote">Save note</button></div>`:""}</div>`}else h+=`<div class="card formCard" style="text-align:center">🏖 Classes are excluded from attendance calculations for this holiday.</div>`;return h}
