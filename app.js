...
function adminStudentPage(uid){
  ...
  const filteredGroups=grouped.filter(g=>!selectedMonth||String(g.date).slice(0,7)===selectedMonth).map(g=>({date:g.date,note:g.note||"",rows:g.rows.filter(r=>{
  ...
  const detailRows=filteredGroups.map(g=>{
    ...
    return `<tr class="adminDateGroup"><th colspan="6">${esc(label)}</th></tr>${g.rows.map((r,ri)=>{
      ...
      const noteCell=ri===0?`<td rowspan="${g.rows.length}">${esc(g.note||'—')}</td>`:'';
...
function reportDetailHtml(grouped){
  return grouped.map(g=>{const n=g.rows.length;return `<section class="reportDay"><h3>${esc(g.label)}</h3><div class="reportTable"><div class="reportTableHead"><span>Scheduled Class</span><span>Actual Class</span><span>Type</span><span>Status</span><span>What changed?</span><span>Note</span></div>${g.rows.map((r,i)=>`<div class="reportTableRow"><span><b>${esc(r.scheduledSubject||"—")}</b><small>${esc(r.scheduled)}</small></span><span><b>${esc(r.actualSubject||"—")}</b><small>${esc(r.actual)}</small></span><span>${esc(r.type)}</span><span>${esc(r.status)}</span><span>${esc(r.relationship||"—")}</span>${i===0?`<span class="reportMergedNote" style="grid-column:6;grid-row:1 / span ${n};align-self:stretch;display:flex;align-items:center;">${esc(g.note||"—")}</span>`:''}</div>`).join("")}</div></section>`}).join("")||'<div class="small">No attendance records found.</div>';
}
