...
h+=`<div class="card noteBox"><h3>📝 Note for this day</h3>${state.notes[dk]?`<div class="savedNotePreview"><b>Saved note</b><div>${esc(state.notes[dk]).replace(/\n/g,"<br>")}</div></div>`:""}<textarea id="dayNote" ${can?"":"disabled"} placeholder="Optional note...">${esc(state.notes[dk]||"")}</textarea>${can?`<div style="margin-top:8px"><button class="saveAttendance actionCompact" data-act="saveNote">Save note</button></div>`:""}</div>`;
