...
function adminStudentCards(students){
  return students.map(s=>`<div class="adminStudent card" role="button" tabindex="0" data-uid="${esc(s.uid)}" onclick="openAdminStudent(this.getAttribute('data-uid'));return false;" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openAdminStudent(this.getAttribute('data-uid'));return false;}"><div class="adminStudentMain"><b>${esc(s.name||"Student")}</b><span>Roll: ${esc(s.rollNumber||"—")}</span></div><div class="small">${esc(s.email||"")}</div><div class="adminStudentMeta"><span>Required ${Number(s.required||75)}%</span><span>${esc(s.course||"")}</span></div></div>`).join('')||`<div class="card formCard"><p class="small">No matching students.</p></div>`;
}
...
function adminStudentPage(uid){
  ...
  days.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    ...
    for(let i=0;i<max;i++){
      const snap=sessions[i]||{},rec=d.records?.[i]||{},status=d.status?.[i]||"pending",info=adminClassInfo(snap,rec,i,sessions);
      rows.push({date:d.date,index:i,status,subject:info.actualSubject,scheduledSubject:info.scheduledSubject,actualSubject:info.actualSubject,scheduledStart:info.scheduledStart,scheduledEnd:info.scheduledEnd,actualStart:info.actualStart,actualEnd:info.actualEnd,note:d.note||"",holiday:d.holiday,type:info.type,relationship:info.relationship});
    }
    if(d.note&&!max)rows.push({date:d.date,index:"",status:"note",subject:"",scheduledSubject:"",actualSubject:"",scheduledStart:"",scheduledEnd:"",actualStart:"",actualEnd:"",note:d.note,holiday:d.holiday,type:"Note",relationship:"Day note"});
...
function buildAdminReportData(s,days){
  ...
  days.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    ...
    for(let i=0;i<max;i++){
      const r=d.records?.[i]||{},snap=sessions[i]||{},status=d.status?.[i]||"pending",info=adminClassInfo(snap,r,i,sessions);
...
function reportDetailHtml(grouped){
  return grouped.map(g=>`<section class="reportDay"><h3>${esc(g.label)}</h3><div class="reportTable"><div class="reportTableHead"><span>Scheduled Class</span><span>Actual Class</span><span>Type</span><span>Status</span><span>What changed?</span><span>Note</span></div>${g.rows.map(r=>`<div class="reportTableRow"><span>${esc(r.scheduledSubject||"—")}<small>${esc(r.scheduled)}</small></span><span>${esc(r.actualSubject||"—")}<small>${esc(r.actual)}</small></span><span>${esc(r.type)}</span><span>${esc(r.status)}</span><span>${esc(r.relationship||"—")}</span><span>${esc(r.note||"—")}</span></div>`).join("")}</div></section>`).join("")||'<div class="small">No attendance records found.</div>';
}
...
async function handleCreateStudentAccount(){
  ...
  try{
    ...
    save();
    document.querySelectorAll('.modal').forEach(m=>m.remove());
    toast('Student account created. Your data will now sync to Firebase.');
...
window.openAdminStudent=uid=>openAdminStudent(uid);
