...
function cloudSyncLabel(){if(cloudSession.syncing)return "Syncing…";if(cloudSession.error)return cloudSession.error;if(!cloudSession.user)return "Not signed in";return cloudSession.lastSync?"✓ Synced · "+cloudSession.lastSync.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}):"Cloud connected";}
function updateCloudStatus(){const el=document.getElementById("cloudStatusText");if(!el)return;el.textContent=cloudSyncLabel();el.className="small "+(cloudSession.error?"danger":cloudSession.user?"safe":"");}
...
function adminStudentCards(students){return students.map(s=>`<button type="button" class="adminStudent card" data-act="adminStudent" data-uid="${esc(s.uid)}" onclick="openAdminStudent('${esc(s.uid)}');return false;"><div class="adminStudentMain"><b>${esc(s.name||"Student")}</b><span>Roll: ${esc(s.rollNumber||"—")}</span></div><div class="small">${esc(s.email||"")}</div><div class="adminStudentMeta"><span>Required ${Number(s.required||75)}%</span><span>${esc(s.course||"")}</span><span class="adminSyncStamp">${esc(adminSyncLabel(s))}</span></div></button>`).join('')||`<div class="card formCard"><p class="small">No matching students.</p></div>`}
