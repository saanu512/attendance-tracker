...
function attendanceAdvice(r){if(r.held===0)return "No held classes yet";let req=Number(state.settings.required)||75, q=req/100;if(q>=1){return r.attended===r.held?"✓ 100% attendance — do not miss any class":"Need to attend all future classes; 100% can only be reached if no class was missed"}if(r.pct>=req){let canMiss=Math.floor(r.attended/q-r.held+1e-9);if(canMiss<0)canMiss=0;return "✓ Attendance is enough — you can miss "+canMiss+" more class"+(canMiss===1?"":"es")+" and still stay at or above "+req+"%"}let needed=Math.ceil((q*r.held-r.attended)/(1-q));if(needed<0)needed=0;return "⚠ Need to attend the next "+needed+" class"+(needed===1?"":"es")+" consecutively to reach "+req+"%"}
