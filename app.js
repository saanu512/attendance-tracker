...
function classTypeForSave(dk,i,session,baseCount){
  ...
  if(!original)return "Actual / Regular";
  ...
  if(actual===os && at===ot)return "Actual / Regular";
...
function settingsTab(){
  ...
  const accountCard=`<div class="card formCard cloudAccountCard"><b>☁️ Account & Cloud</b><div class="cloudIdentity">${user?`<div class="cloudAvatar">${cloudSession.admin?"👑":esc((user.email||"S").slice(0,1).toUpperCase())}</div><div class="grow"><b>${esc(user.email||"")}</b><div id="cloudStatusText" class="small ${cloudSession.error?"danger":"safe"}">${cloudSession.admin?"Admin account":cloudSession.syncing?"Syncing…":cloudSession.error?esc(cloudSession.error):cloudSession.lastSync?"Synced · "+cloudSession.lastSync.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}):"Cloud connected"}</div></div>`:`<div class="cloudAvatar">☁</div><div class="grow"><b>Not signed in</b><div id="cloudStatusText" class="small">Sign in to save data permanently to Firebase.</div></div>`}</div><div class="cloudActions">${user?`${cloudSession.admin?`<button class="btn soft" data-act="adminDashboard">👑 Admin Dashboard</button>`:`<button class="btn soft" data-act="syncNow">☁️ Sync Now</button>`}<button class="btn outline" data-act="logout">Logout</button>`:`<button class="btn" data-act="studentLogin">Student Login</button><button class="btn outline" data-act="adminLogin">Admin Login</button>`}</div>${user?`<div class="cloudActions secondary"><button class="btn outline" data-act="forgotPassword">Forgot Password</button></div>`:`<p class="small">Student and Admin accounts use Firebase Email/Password authentication. Admin access requires an authorized Firebase admin role.</p>`}</div>`;
...
function adminStudentPage(uid){
  ...
  days.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    ...
    for(let i=0;i<max;i++){
      const snap=sessions[i]||{},rec=d.records?.[i]||{},status=d.status?.[i]||"pending";
      const subject=rec.subject||snap.subject||rec.scheduled||snap.scheduledSubject||"Scheduled";
      rows.push({date:d.date,index:i,status,subject,scheduled:rec.scheduled||snap.scheduledSubject||subject,start:rec.start||snap.start||"",end:rec.end||snap.end||"",scheduledStart:rec.scheduledStart||snap.start||"",scheduledEnd:rec.scheduledEnd||snap.end||"",note:d.note||"",holiday:d.holiday,type:rec.type||classTypeForSnapshot(snap,rec,i,sessions)});
    }
    if(d.note&&!max)rows.push({date:d.date,index:"",status:"note",subject:"",scheduled:"",start:"",end:"",scheduledStart:"",scheduledEnd:"",note:d.note,holiday:d.holiday,type:"Note"});
  ...
  const detailRows=grouped.map(g=>{
    ...
    return `<tr class="adminDateGroup"><th colspan="5">${esc(label)}</th></tr>${g.rows.map(r=>{
      const scheduled=r.scheduledStart&&r.scheduledEnd?time(r.scheduledStart)+'–'+time(r.scheduledEnd):'—';
      const actual=r.start&&r.end?time(r.start)+'–'+time(r.end):'';
      const timeCell=actual&&actual!==scheduled?`${esc(scheduled)}<small>Actual: ${esc(actual)}</small>`:esc(scheduled);
      return `<tr><td>${timeCell}</td><td>${esc(r.subject||'—')}</td><td>${esc(r.type||'Regular')}</td><td>${esc(adminStatusLabel(r.status))}</td><td>${esc(r.note||'')}</td></tr>`;
    }).join('')}`;
  }).join('')||`<tr><td colspan="5" class="small">No attendance records found.</td></tr>`;
  let h=`<div class="card formCard adminHero"><div class="row" style="justify-content:space-between"><div><b>👤 ${esc(s.name||"Student")}</b><div class="small">Roll ${esc(s.rollNumber||"—")} · ${esc(s.email||"")}</div></div><button class="btn outline" data-act="backFromStudent">Back</button></div><div class="adminSummaryGrid"><div><b>${stats.present}</b><span>Present</span></div><div><b>${stats.absent}</b><span>Absent</span></div><div><b>${stats.pending}</b><span>Pending</span></div><div><b>${stats.total}</b><span>Total</span></div></div><div class="adminPercent">Overall attendance: <b>${stats.percent===null?'—':stats.percent.toFixed(1)+'%'}</b></div></div><div class="card formCard"><b>📚 Subject-wise attendance</b>${Object.entries(stats.subjects).sort((a,b)=>a[0].localeCompare(b[0])).map(([sub,x])=>`<div class="adminSubjectRow"><b>${esc(sub)}</b><span>${x.held?((x.present/x.held)*100).toFixed(1)+'%':'—'}</span><small>Present ${x.present} · Absent ${x.absent} · Pending ${x.pending} · Not Held ${x.notHeld} · Total ${x.total}</small></div>`).join('')||`<div class="small" style="margin-top:10px">No subject records yet.</div>`}</div><div class="card formCard adminDetailsCard"><div class="adminDetailsHead"><b>📋 Detailed attendance</b><button class="btn outline" data-act="printAdminReport">View Full Report</button></div><details class="adminDetails" open><summary>Show records grouped by date</summary><div class="adminTableWrap"><table class="adminTable adminGroupedTable"><thead><tr><th>Time</th><th>Subject</th><th>Type</th><th>Status</th><th>Note</th></tr></thead><tbody>${detailRows}</tbody></table></div></details></div>`;
  return h;
}
function classTypeForSnapshot(snap,rec,i,sessions){if(rec?.type)return rec.type;if(snap?.extra)return "Extra";const scheduled=snap?.scheduledSubject||rec?.scheduled||snap?.subject||rec?.subject||"";const actual=snap?.subject||rec?.subject||scheduled;const st=(snap?.start||rec?.start||"")+(snap?.end||rec?.end||"");const ot=(snap?.start||"")+(snap?.end||"");if(actual===scheduled&&st===ot)return "Actual / Regular";if(actual===scheduled)return "Rescheduled";const others=(sessions||[]).filter(x=>!x.extra).map(x=>x.scheduledSubject||x.subject);return others.includes(actual)?"Exchange / Replacement":"Replacement"}
function classTypeForAdmin(rec,d,i){if(rec?.type)return rec.type;if(rec?.scheduled&&rec.subject&&rec.scheduled!==rec.subject){if(rec.scheduledStart&&rec.start&&rec.scheduledStart!==rec.start)return rec.subject===rec.scheduled?"Rescheduled":"Replacement";return "Replacement"}return "Actual / Regular"}
...
function printAdminReport(){
  ...
  days.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    ...
    const rows=[];
    for(let i=0;i<max;i++){
      const r=d.records?.[i]||{},snap=sessions[i]||{},status=d.status?.[i]||"pending";
      const subject=r.subject||snap.subject||r.scheduled||snap.scheduledSubject||"Scheduled";
      const scheduled=(r.scheduledStart&&r.scheduledEnd)?time(r.scheduledStart)+"–"+time(r.scheduledEnd):(snap.start&&snap.end?time(snap.start)+"–"+time(snap.end):"—");
      const actual=(r.start&&r.end)?time(r.start)+"–"+time(r.end):"";
      rows.push({scheduled,actual,subject,type:classTypeForSnapshot(snap,r,i,sessions),status:adminStatusLabel(status),note:d.note||""});
    }
    if(d.note&&!rows.length) rows.push({scheduled:"—",actual:"",subject:"Day note",type:"Note",status:"Note",note:d.note});
