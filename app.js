...
let cloudSession={user:null,admin:false,ready:!!window.AttendanceCloud,syncing:false,lastSync:null,error:"",syncHealthy:false};
...
let adminCache={students:[],selected:null,days:[],month:null,statusFilter:"all"};
...
function cloudQueue(label,date){
  ...
  cloudSyncChain=cloudSyncChain.then(async()=>{
    ...
    try{
      ...
      rememberCloudSync(cloudSession.user.uid);
      cloudSession.error="";
...
function cloudQueueFull(label){
  ...
  cloudSyncChain=cloudSyncChain.then(async()=>{
    ...
    try{
      ...
      rememberCloudSync(cloudSession.user.uid); cloudSession.error="";
    }catch(e){cloudSession.error=cloudErrorText(e);throw e}
    finally{cloudSession.syncing=false;updateCloudStatus();}
  }).then(()=>{localStorage.removeItem(CLOUD_PENDING_KEY);updateCloudStatus()}).catch(()=>{updateCloudStatus()});
  return cloudSyncChain;
...
function cloudLastSyncKey(uid){return "attendance-tracker-last-sync:"+String(uid||"");}
function rememberCloudSync(uid,date=new Date()){if(!uid||!(date instanceof Date)||Number.isNaN(date.getTime()))return;localStorage.setItem(cloudLastSyncKey(uid),date.toISOString());cloudSession.lastSync=date;cloudSession.syncHealthy=true;}
function restoreCloudSync(uid,fallback){if(!uid)return null;let raw=localStorage.getItem(cloudLastSyncKey(uid));let d=raw?new Date(raw):null;if(!(d instanceof Date)||Number.isNaN(d?.getTime?.()))d=null;if(!d&&fallback){d=new Date(fallback);if(Number.isNaN(d.getTime()))d=null;}cloudSession.lastSync=d;cloudSession.syncHealthy=!!d;return d;}
function cloudSyncState(){if(!cloudSession.user)return {label:"● Not signed in",cls:"danger"};if(cloudSession.error)return {label:"● Sync failed",cls:"danger"};if(cloudSession.syncing)return {label:"● Syncing…",cls:"safe"};if(cloudSession.syncHealthy)return {label:"● Cloud sync working",cls:"safe"};return {label:"● Cloud sync not confirmed",cls:"danger"};}
function cloudSyncLabel(){const st=cloudSyncState();return st.label;}
function cloudLastSyncLabel(){return cloudSession.lastSync?"Last successful cloud sync: "+cloudSession.lastSync.toLocaleString([], {day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}):"Last successful cloud sync: —";}
function updateCloudStatus(){const el=document.getElementById("cloudStatusText");if(!el)return;const st=cloudSyncState();el.innerHTML=`<span class="cloudSyncState ${st.cls}">${st.label}</span><span class="cloudLastSync">${esc(cloudLastSyncLabel())}</span>`;el.className="cloudStatusBlock";}
...
function settingsTab(){
  ...
  const accountCard=`<div class="card formCard cloudAccountCard"><b>☁️ Account & Cloud</b><div class="cloudIdentity card">${user?`<div class="cloudAvatar">${cloudSession.admin?"👑":esc((user.email||"S").slice(0,1).toUpperCase())}</div><div class="grow"><b>${esc(user.email||"")}</b><div class="small">${cloudSession.admin?"Admin account":"Firebase account"}</div></div>`:`<div class="cloudAvatar">☁</div><div class="grow"><b>Not signed in</b><div class="small">Sign in to save data permanently to Firebase.</div></div>`}</div><div id="cloudStatusText" class="cloudStatusBlock">${(()=>{const st=cloudSyncState();return `<span class="cloudSyncState ${st.cls}">${st.label}</span><span class="cloudLastSync">${esc(cloudLastSyncLabel())}</span>`})()}</div><div class="cloudActions">${user?`${cloudSession.admin?`<button class="btn soft" data-act="adminDashboard">👑 Admin Dashboard</button>`:`<button class="btn soft" data-act="syncNow">☁️ Sync Now</button>`}<button class="btn outline" data-act="logout">Logout</button>`:`<button class="btn" data-act="studentLogin">Student Login</button><button class="btn outline" data-act="adminLogin">Admin Login</button>`}</div>${user?`<div class="cloudActions secondary"><button class="btn outline" data-act="forgotPassword">Forgot Password</button></div>`:`<p class="small">Student and Admin accounts use Firebase Email/Password authentication. Admin access requires an authorized Firebase admin role.</p>`}</div>`;
...
function adminDateLabel(iso){const d=parseKey(iso);return `${DAYS[d.getDay()]}, ${fmt(d)}`;}
function adminMonthKey(iso){const d=parseKey(iso);return `${d.getFullYear()}-${pad(d.getMonth()+1)}`;}
function adminMonthLabel(mk){const [y,m]=String(mk).split("-").map(Number);return new Date(y,m-1,1).toLocaleDateString("en-GB",{month:"long",year:"numeric"});}
function adminSyncLabel(s){
  const raw=s?.updatedAt;
  if(!raw)return "Last cloud sync: —";
  const d=new Date(raw);
  return Number.isNaN(d.getTime())?"Last cloud sync: —":"Last cloud sync: "+d.toLocaleString([], {day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"});
}
function adminDashboard(){
  ...
  const refreshed=adminCache.lastRefresh?new Date(adminCache.lastRefresh).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}):"—";
  let h=`<div class="card formCard adminHero"><div class="row" style="justify-content:space-between"><div><b>👑 Admin Dashboard</b><div class="small">Complete cloud student records</div><div class="adminCloudStatus"><span>☁️ Firebase connected</span><span>Last dashboard refresh: ${esc(refreshed)}</span></div></div><button class="btn outline" data-act="backFromAdmin">Back</button></div><div class="adminSearch"><input id="adminSearch" class="input" placeholder="Search name, roll or email" oninput="filterAdminStudents(this.value)"></div></div><div id="adminStudentList">${adminStudentCards(adminCache.students)}</div>`;return h;
}
function adminStudentCards(students){return students.map(s=>`<button type="button" class="adminStudent card" data-act="adminStudent" data-uid="${esc(s.uid)}"><div class="adminStudentMain"><b>${esc(s.name||"Student")}</b><span>Roll: ${esc(s.rollNumber||"—")}</span></div><div class="small">${esc(s.email||"")}</div><div class="adminStudentMeta"><span>Required ${Number(s.required||75)}%</span><span>${esc(s.course||"")}</span></div></button>`).join('')||`<div class="card formCard"><p class="small">No matching students.</p></div>`}
...
function adminMonthPicker(days){
  const months=[...new Set(days.map(d=>adminMonthKey(d.date)))].sort().reverse();
  if(!months.length)return `<div class="adminMonthCard card"><b>🗓️ Attendance history</b><p class="small">No attendance records found.</p></div>`;
  return `<div class="adminMonthCard card"><div class="adminMonthHead"><b>🗓️ Attendance history</b><span class="small">Select a month to view daily records</span></div><div class="adminMonthList">${months.map(m=>`<button class="adminMonthChoice btn outline" data-act="adminMonth" data-v="${m}"><span>${esc(adminMonthLabel(m))}</span><span>›</span></button>`).join('')}</div></div>`;
}
...
function adminStatusFilterLabel(v){return v==="all"?"All":v==="attended"?"Present":v==="absent"?"Absent":v==="not_held"?"Not Held":v==="extra"?"Extra Classes":v==="pending"?"Pending":"All";}
function adminStatusMatches(filter,row){
  if(filter==="all")return true;
  if(filter==="extra")return row.type==="Extra";
  if(filter==="absent")return row.status==="absent"||row.status==="leave";
  return row.status===filter;
}
function adminStatusFilter(days,active){
  const items=[['all','All'],['attended','Present'],['absent','Absent'],['not_held','Not Held'],['extra','Extra Classes'],['pending','Pending']];
  return `<div class="adminStatusFilter card"><div class="adminFilterTitle"><b>🔎 Filter classes</b><span class="small">${esc(active==="all"?"Showing all classes":"Showing: "+adminStatusFilterLabel(active))}</span></div><div class="adminFilterChips">${items.map(([v,l])=>`<button class="filter ${active===v?"active":""}" data-act="adminStatusFilter" data-v="${v}">${l}</button>`).join('')}</div></div>`;
}
function adminStudentPage(uid){
  ...
  const month=adminCache.month;
  const statusFilter=adminCache.statusFilter||"all";
  const monthDays=month?days.filter(d=>adminMonthKey(d.date)===month):[];
  const grouped=[];
  monthDays.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    ...
    for(let i=0;i<max;i++){
      ...
      const row={date:d.date,index:i,status,scheduledSubject:info.scheduledSubject,actualSubject:info.actualSubject,scheduledStart:info.scheduledStart,scheduledEnd:info.scheduledEnd,actualStart:info.actualStart,actualEnd:info.actualEnd,note:d.note||"",holiday:d.holiday,type:info.type,relationship:info.relationship};
      if(adminStatusMatches(statusFilter,row))rows.push(row);
    }
    if(d.note&&!max && adminStatusMatches(statusFilter,{status:"note",type:"Note"}) && statusFilter==="all")rows.push({date:d.date,index:"",status:"note",scheduledSubject:"—",actualSubject:"Day note",scheduledStart:"",scheduledEnd:"",actualStart:"",actualEnd:"",note:d.note,holiday:d.holiday,type:"Note",relationship:"Day note"});
    if(rows.length)grouped.push({date:d.date,rows});
  });
  const detailRows=grouped.map(g=>{
    const label=adminDateLabel(g.date);
  ...
  }).join('')||`<tr><td colspan="6" class="small">No ${esc(statusFilterLabel(statusFilter).toLowerCase())} classes found for ${esc(month?adminMonthLabel(month):"this month")}.</td></tr>`;
  const detailSection=month?`${adminStatusFilter(monthDays,statusFilter)}<div class="card formCard adminDetailsCard"><div class="adminDetailsHead"><div><b>📋 ${esc(adminMonthLabel(month))} attendance</b><div class="small">Daily attendance for this month only · ${esc(statusFilterLabel(statusFilter))}</div></div><button class="btn outline" data-act="adminMonths">← All months</button></div><div class="adminTableWrap"><table class="adminTable adminGroupedTable"><thead><tr><th>Scheduled Class</th><th>Actual Class</th><th>Type</th><th>Status</th><th>What changed?</th><th>Note</th></tr></thead><tbody>${detailRows}</tbody></table></div></div>`:adminMonthPicker(days);
  return `<div class="card formCard adminHero"><div class="row" style="justify-content:space-between"><div><b>👤 ${esc(s.name||"Student")}</b><div class="small">Roll ${esc(s.rollNumber||"—")} · ${esc(s.email||"")}</div><div class="adminStudentCloudStatus">☁️ ${esc(adminSyncLabel(s))}</div></div><button class="btn outline" data-act="backFromStudent">Back</button></div><div class="adminSummaryGrid"><div class="card"><b>${stats.present}</b><span>Present</span></div><div class="card"><b>${stats.absent}</b><span>Absent</span></div><div class="card"><b>${stats.pending}</b><span>Pending</span></div><div class="card"><b>${stats.total}</b><span>Total</span></div></div><div class="adminPercent">Overall attendance: <b>${stats.percent===null?'—':stats.percent.toFixed(1)+'%'}</b></div></div><div class="card formCard"><b>📚 Subject-wise attendance</b>${Object.entries(stats.subjects).sort((a,b)=>a[0].localeCompare(b[0])).map(([sub,x])=>`<div class="adminSubjectRow"><b>${esc(sub)}</b><span>${x.held?((x.present/x.held)*100).toFixed(1)+'%':'—'}</span><small>Present ${x.present} · Absent ${x.absent} · Pending ${x.pending} · Not Held ${x.notHeld} · Total ${x.total}</small></div>`).join('')||`<div class="small" style="margin-top:10px">No subject records yet.</div>`}</div><div class="adminReportActions adminReportActionsTop"><button class="btn outline" data-act="printAdminReport">View Full Report</button><button class="btn soft" data-act="exportAdminPdf">Export PDF</button></div>${detailSection}`;
}
...
function printAdminReport(){
  ...
  modal.innerHTML=`<div class="modalBox reportSheet"><div class="reportHead card"><div><h2>📋 Attendance Report</h2><div class="small">Complete cloud record</div></div><button class="btn outline" data-act="closeModal">Close</button></div><div class="reportIdentity"><b>${esc(s.name||"Student")}</b><span>Roll ${esc(s.rollNumber||"—")} · ${esc(s.email||"")}</span><span>Required attendance: ${Number(s.required||75)}%</span></div><div class="reportMetrics"><div class="card"><b>${stats.present}</b><span>Present</span></div><div class="card"><b>${stats.absent}</b><span>Absent</span></div><div class="card"><b>${stats.pending}</b><span>Pending</span></div><div class="card"><b>${stats.total}</b><span>Total</span></div></div><div class="reportOverall">Overall attendance: <b>${stats.percent===null?"—":stats.percent.toFixed(1)+"%"}</b></div><h3 class="reportSectionTitle">📚 Subject-wise attendance</h3><div class="reportSubjects">${reportSubjectHtml(stats)}</div><h3 class="reportSectionTitle">🗓️ Detailed attendance</h3><div class="reportDetails">${reportDetailHtml(grouped)}</div></div>`;
...
async function handleCloudLogout(){
  ...
  adminCache={students:[],selected:null,days:[],month:null,statusFilter:"all",lastRefresh:null};
  save();
  applyTheme();
  render();
  toast("Logged out");
...
async function hydrateAfterLogin(user){
  try{
    ...
    restoreCloudSync(user.uid, profile?.updatedAt);
    updateCloudStatus();
    ...
    if(profile){
      ...
      await window.AttendanceCloud.markMigrationComplete(user.uid);
      localStorage.setItem(CLOUD_MIGRATION_KEY+":"+user.uid,"1");
      rememberCloudSync(user.uid);updateCloudStatus();
      toast("Cloud account created from this device's saved data");
    }
  }catch(e){cloudSession.error=cloudErrorText(e);cloudSession.syncHealthy=false;updateCloudStatus();toast("Cloud sync failed — see Account & Cloud for the exact Firebase error")}
}
...
async function openAdminDashboard(){
  if(!cloudSession.admin)return;state.adminPage="dashboard";adminCache.selected=null;adminCache.month=null;adminCache.statusFilter="all";adminCache.lastRefresh=null;render();
  try{adminCache.students=await window.AttendanceCloud.listStudents();adminCache.lastRefresh=new Date().toISOString();render()}catch(e){cloudSession.error=e?.message||"Admin data could not be loaded";toast("Could not load student data")}
}
async function openAdminStudent(uid){
  state.adminPage="student";adminCache.selected=uid;adminCache.days=[];adminCache.month=null;adminCache.statusFilter="all";render();
...
function setupFirebaseEvents(){
  ...
  window.addEventListener("firebase-auth-state",async e=>{const user=e.detail?.user||null;cloudSession.user=user;cloudSession.admin=!!e.detail?.admin;if(!user){cloudSession.error="";cloudSession.lastSync=null;cloudSession.syncHealthy=false}if(user&&!cloudSession.admin){cloudSession.error="";await hydrateAfterLogin(user)}else{cloudSession.lastSync=user?restoreCloudSync(user.uid):null;state.adminPage=null;adminCache={students:[],selected:null,days:[],month:null,statusFilter:"all",lastRefresh:null};render()}updateCloudStatus();render()});
