...
function applyTheme(){const ed=(state.settings&&state.settings.edition)||"earth-day";document.body.classList.toggle("nebula-glass",ed==="nebula-glass");document.body.classList.toggle("galactic-night",ed==="galactic-night");document.body.classList.toggle("black-hole",ed==="black-hole");document.body.classList.toggle("heart-nebula",ed==="heart-nebula");document.body.classList.toggle("roshni",ed==="roshni");document.body.classList.toggle("opal-dream",ed==="opal-dream");document.body.setAttribute("data-edition",ed);
...
load();applyTheme();watchSystemTheme();render();if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",activateGate,{once:true});}else{activateGate();}if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=48-opal-dream").then(r=>r.update?.()).catch(()=>{}));
