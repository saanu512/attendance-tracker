...
function render(){applyTheme();let content=state.infoPage?dailyInfoPage():state.tab==="log"?logTab():state.tab==="stats"?statsTab():state.tab==="calendar"?calendarTab():settingsTab();let animate=window.__animateNavigation===true;window.__animateNavigation=false;document.getElementById("app").innerHTML=`<div class="app">${header()}<main class="container${animate?" tabTransition":""}">${content}</main>${state.infoPage?"":bottom()}</div>`}
