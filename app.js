...
const CLOUD_LAST_SYNC_KEY="attendance-tracker-last-cloud-sync";
let cloudSession={user:null,admin:false,ready:!!window.AttendanceCloud,syncing:false,lastSync:(()=>{try{const v=localStorage.getItem(CLOUD_LAST_SYNC_KEY);return v?new Date(v):null}catch(_){return null}})(),error:""};
...
let adminCache={students:[],selected:null,days:[],selectedMonth:null,statusFilter:"all"};
...
function updateCloudStatus(){const el=document.getElementById("cloudStatusText");if(!el)return;let text="",cls="small";if(cloudSession.syncing){text="✓ Syncing…";cls+=" safe"}else if(cloudSession.error){text="✕ Sync failed · "+cloudSession.error;cls+=" danger"}else if(cloudSession.user){text=cloudSession.lastSync?"✓ Synced · Last synced "+cloudSession.lastSync.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit",second:"2-digit"}):"✓ Cloud connected · Last synced: not yet";cls+=" safe"}else{text="✕ Not signed in";cls+=" danger"}el.textContent=text;el.className=cls;}
function markCloudSyncSuccess(){cloudSession.lastSync=new Date();cloudSession.error="";try{localStorage.setItem(CLOUD_LAST_SYNC_KEY,cloudSession.lastSync.toISOString())}catch(_){}updateCloudStatus();}
...
function settingsTab(){
  ...
  const accountCard=`<div class="card formCard cloudAccountCard"><b>☁️ Account & Cloud</b><div class="cloudIdentity">${user?`<div class="cloudAvatar">${cloudSession.admin?"👑":esc((user.email||"S").slice(0,1).toUpperCase())}</div><div class="grow"><b>${esc(user.email||"")}</b><div id="cloudStatusText" class="small ${cloudSession.error?"danger":"safe"}">${cloudSession.admin?"✓ Admin account":cloudSession.syncing?"✓ Syncing…":cloudSession.error?"✕ Sync failed":cloudSession.lastSync?"✓ Synced · Last synced "+cloudSession.lastSync.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit",second:"2-digit"}):"✓ Cloud connected · Last synced: not yet"}</div></div>`:`<div class="cloudAvatar">☁</div><div class="grow"><b>Not signed in</b><div id="cloudStatusText" class="small">Sign in to save data permanently to Firebase.</div></div>`}</div><div class="cloudActions">${user?`${cloudSession.admin?`<button class="btn soft" data-act="adminDashboard">👑 Admin Dashboard</button>`:`<button class="btn soft" data-act="syncNow">☁️ Sync Now</button>`}<button class="btn outline" data-act="logout">Logout</button>`:`<button class="btn" data-act="studentLogin">Student Login</button><button class="btn outline" data-act="adminLogin">Admin Login</button>`}</div>${user?`<div class="cloudActions secondary"><button class="btn outline" data-act="forgotPassword">Forgot Password</button></div>`:`<p class="small">Student and Admin accounts use Firebase Email/Password authentication. Admin access requires an authorized Firebase admin role.</p>`}</div>`;
...
function adminStudentPage(uid){
  ...
  const months=[...new Set(days.map(d=>String(d.date||"").slice(0,7)).filter(Boolean))].sort().reverse();
  const selectedMonth=adminCache.selectedMonth&&months.includes(adminCache.selectedMonth)?adminCache.selectedMonth:null;
  const statusFilter=adminCache.statusFilter||"all";
  const monthButtons=months.length?months.map(m=>{const d=parseKey(m+"-01");return `<button class="filter ${selectedMonth===m?"active":""}" data-act="adminMonth" data-v="${m}">${esc(monthFmt(d))}</button>`}).join(""):`<div class="small">No dated cloud records found.</div>`;
  const filterButtons=[["all","All"],["attended","Present"],["absent","Absent"],["not_held","Not Held"],["extra","Extra Classes"],["pending","Pending"]].map(([v,l])=>`<button class="filter ${statusFilter===v?"active":""}" data-act="adminStatus" data-v="${v}">${l}</button>`).join("");
  let grouped=[];
  if(selectedMonth){days.slice().sort((a,b)=>String(a.date).localeCompare(String(b.date))).filter(d=>String(d.date).slice(0,7)===selectedMonth).forEach(d=>{
    const rows=[],sessions=d.sessions||[],max=Math.max(sessions.length,Object.keys(d.records||{}).length,Object.keys(d.status||{}).length);
    for(let i=0;i<max;i++){const snap=sessions[i]||{},rec=d.records?.[i]||{},status=d.status?.[i]||"pending",info=adminClassInfo(d.date,snap,rec,i,sessions),type=info.type||"Regular";const matches=statusFilter==="all"||(statusFilter==="extra"?type==="Extra":statusFilter==="absent"?(status==="absent"||status==="leave"):status===statusFilter);if(!matches)continue;rows.push({date:d.date,index:i,status,scheduledSubject:info.scheduledSubject,actualSubject:info.actualSubject,scheduledStart:info.scheduledStart,scheduledEnd:info.scheduledEnd,actualStart:info.actualStart,actualEnd:info.actualEnd,note:d.note||"",holiday:d.holiday,type,relationship:info.relationship})}
    if(d.note&&!max&&statusFilter==="all")rows.push({date:d.date,index:"",status:"note",scheduledSubject:"—",actualSubject:"Day note",scheduledStart:"",scheduledEnd:"",actualStart:"",actualEnd:"",note:d.note,holiday:d.holiday,type:"Note",relationship:"Day note"});
    if(rows.length)grouped.push({date:d.date,rows});
  })}
  const stats=adminCompute(days);
  const detailRows=grouped.map(g=>{const d=parseKey(g.date),label=`${DAYS[d.getDay()]}, ${fmt(d)}`;return `<tr class="adminDateGroup"><th colspan="6">${esc(label)}</th></tr>${g.rows.map(r=>{const scheduledTime=r.scheduledStart&&r.scheduledEnd?time(r.scheduledStart)+'–'+time(r.scheduledEnd):'—',actualTime=r.actualStart&&r.actualEnd?time(r.actualStart)+'–'+time(r.actualEnd):'—',scheduled=r.scheduledSubject?`${esc(r.scheduledSubject)}<small>${esc(scheduledTime)}</small>`:'—',actual=r.actualSubject?`${esc(r.actualSubject)}<small>${esc(actualTime)}</small>`:'—';return `<tr><td>${scheduled}</td><td>${actual}</td><td>${esc(r.type||'Regular')}</td><td>${esc(adminStatusLabel(r.status))}</td><td>${esc(r.relationship||'—')}</td><td>${esc(r.note||'—')}</td></tr>`}).join('')}`}).join('')||`<tr><td colspan="6" class="small">${selectedMonth?"No records match this month/filter.":"Select a month above to view daily attendance."}</td></tr>`;
  return `<div class="card formCard adminHero"><div class="row" style="justify-content:space-between"><div><b>👤 ${esc(s.name||"Student")}</b><div class="small">Roll ${esc(s.rollNumber||"—")} · ${esc(s.email||"")}</div></div><button class="btn outline" data-act="backFromStudent">Back</button></div><div class="adminSummaryGrid"><div><b>${stats.present}</b><span>Present</span></div><div><b>${stats.absent}</b><span>Absent</span></div><div><b>${stats.pending}</b><span>Pending</span></div><div><b>${stats.total}</b><span>Total</span></div></div><div class="adminPercent">Overall attendance: <b>${stats.percent===null?'—':stats.percent.toFixed(1)+'%'}</b></div></div><div class="card formCard"><b>📅 Select month</b><div class="tabs adminMonthTabs">${monthButtons}</div>${selectedMonth?`<div class="small" style="margin-top:8px">Showing ${esc(monthFmt(parseKey(selectedMonth+"-01")))}.</div>`:""}</div>${selectedMonth?`<div class="card formCard"><b>🔎 Filter attendance</b><div class="tabs adminStatusTabs">${filterButtons}</div></div>`:""}<div class="card formCard"><b>📚 Subject-wise attendance</b>${Object.entries(stats.subjects).sort((a,b)=>a[0].localeCompare(b[0])).map(([sub,x])=>`<div class="adminSubjectRow"><b>${esc(sub)}</b><span>${x.held?((x.present/x.held)*100).toFixed(1)+'%':'—'}</span><small>Present ${x.present} · Absent ${x.absent} · Pending ${x.pending} · Not Held ${x.notHeld} · Total ${x.total}</small></div>`).join('')||`<div class="small" style="margin-top:10px">No subject records yet.</div>`}</div><div class="card formCard adminDetailsCard"><div class="adminDetailsHead"><b>📋 Detailed attendance${selectedMonth?` · ${esc(monthFmt(parseKey(selectedMonth+"-01")))}`:""}</b><div class="adminReportActions"><button class="btn outline" data-act="printAdminReport">View Full Report</button><button class="btn soft" data-act="exportAdminPdf">Export PDF</button></div></div><details class="adminDetails" open><summary>${selectedMonth?`Show ${statusFilter==="all"?"all":statusFilter.replace("_"," ")} records grouped by date`:`Select a month to show daily records`}</summary><div class="adminTableWrap"><table class="adminTable adminGroupedTable"><thead><tr><th>Scheduled Class</th><th>Actual Class</th><th>Type</th><th>Status</th><th>What changed?</th><th>Note</th></tr></thead><tbody>${detailRows}</tbody></table></div></details></div>`;
}
...
async function openAdminStudent(uid){
  state.adminPage="student";adminCache.selected=uid;adminCache.days=[];adminCache.selectedMonth=null;adminCache.statusFilter="all";render();
...
function setupFirebaseEvents(){
  ...
  window.addEventListener("firebase-auth-state",async e=>{const user=e.detail?.user||null;cloudSession.user=user;cloudSession.admin=!!e.detail?.admin;if(!user)cloudSession.error="";if(user&&!cloudSession.admin){cloudSession.error="";await hydrateAfterLogin(user)}else{state.adminPage=null;adminCache={students:[],selected:null,days:[],selectedMonth:null,statusFilter:"all"};render()}updateCloudStatus();render()});
