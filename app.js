...
function updateCloudStatus(){const el=document.getElementById("cloudStatusText");if(!el)return;const st=cloudSyncState();el.innerHTML=`<span class="cloudSyncState ${st.cls}">${st.label}</span><br><span class="cloudLastSync">${esc(cloudLastSyncLabel())}</span>`;el.className="small cloudStatusBlock";}
