...
if(ss.length){h+=`<button class="holiday" data-act="holiday" ${can?"":"disabled"}>${isHoliday?"🏖 Marked as holiday"+(can?" — tap to undo":""):can?"🏖 Mark whole day as holiday":"🔒 Not a holiday"}</button>`;if(!isHoliday){ss.forEach((s,i)=>{let cur=draftValue(sessionKey(dk,i)),classCan=attendanceEditable(dk,i),locked=classCan?"":"disabled",changed=s._scheduled&&s._scheduled!==s[2];h+=`<div class="card session"><div class="sessionTop"><div><div class="sessionName">${esc(s[2])}${s[3]?" 📌":""}${changed?" 🔄":""}</div>${changed?`<div class="small">Scheduled: ${esc(s._scheduled)}</div>`:""}<div class="sessionTime">${time(s[0])} – ${time(s[1])}</div></div><div>${classCan?`${s[3]?`<div style="display:flex;gap:6px;justify-content:flex-end"><button class="btn outline" style="padding:7px 9px;font-size:11px" type="button" data-act="editClass" data-i="${i}" onclick="classModal(${i})">✏️ Edit</button></div>`:`<button class="btn outline" style="padding:7px 9px;font-size:11px" type="button" data-act="editClass" data-i="${i}" onclick="classModal(${i})">✏️ Edit</button>`}`:`<div class="locktag">${past?"🔒 Saved & locked":future?"🔒 Future":"🕕 Opens later"}</div>`}</div></div><div class="statusRow"><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="attended" class="status green ${cur==="attended"?"active":""}">✓<br>Present</button><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="absent" class="status red ${cur==="absent"?"active":""}">✕<br>Absent</button><button ${locked} data-act="status" data-k="${sessionKey(dk,i)}" data-v="not_held" class="status gray ${cur==="not_held"?"active":""}">⊘<br>Not held</button></div></div>`});if(can)h+=`<div class="attendanceActions"><button class="saveAttendance actionCompact" data-act="saveAttendance">💾 Save attendance</button></div>`}else h+=`<div class="card formCard" style="text-align:center">🏖 Classes are excluded from attendance calculations for this holiday.</div>`}else h+=`<div class="card formCard" style="text-align:center;color:var(--soft)"><div style="font-weight:850;font-size:17px">No classes scheduled</div><div style="margin-top:6px">You can add an extra class for this date.</div></div>`;
h+=`<div class="card noteBox"><h3>📝 Note for this day</h3><textarea id="dayNote" ${can?"":"disabled"} placeholder="Optional note...">${esc(state.notes[dk]||"")}</textarea>${can?`<div style="margin-top:8px"><button class="saveAttendance actionCompact" data-act="saveNote">Save note</button></div>`:""}</div>`;
if(can)h+=`<div class="extraBelowNote"><button class="saveAttendance actionCompact" data-act="addExtra">＋ Add extra class</button><div class="madeby">Made by Sharad Sourav 🩺</div></div>`;else h+=`<div class="extraBelowNote"><div class="card formCard" style="text-align:center;color:var(--soft)">🔒 Extra classes can be added when this date becomes editable.</div><div class="madeby">Made by Sharad Sourav 🩺</div></div>`;return h}
...
function footer(){return state.tab==="log"?``:`<div class="madeby">Made By Sharad Sourav🩺</div>`}
