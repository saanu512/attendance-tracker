...
function printAdminReport(){const s=adminCache.students.find(x=>x.uid===adminCache.selected),days=adminCache.days||[];if(!s){toast("Student report not found");return}try{const blob=buildAdminPdf(s,days),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`Attendance-${(s.rollNumber||"student")}-${key(today())}.pdf`;a.style.display="none";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);toast("PDF generated — check your Downloads") }catch(e){console.error(e);toast("PDF generation failed")}}
