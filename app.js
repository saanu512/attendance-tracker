...
save();
applyTheme();watchSystemTheme();document.body.classList.add("startup-stable");render();if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",activateGate,{once:true});}else{activateGate();}window.addEventListener("resize",centerOpenModal);window.visualViewport&&window.visualViewport.addEventListener("resize",centerOpenModal);if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw.js?v=90").catch(()=>{}));
