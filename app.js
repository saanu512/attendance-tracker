...
function settingsTab(){
  ...
  const accountCard=`<div class="card formCard cloudAccountCard"><b>☁️ Account & Cloud</b><div class="cloudIdentity">${user?`<div class="cloudAvatar">${cloudSession.admin?"👑":esc((user.email||"S").slice(0,1).toUpperCase())}</div><div class="grow"><b>${esc(user.email||"")}</b><div id="cloudStatusText" class="small ${cloudSession.error?"danger":"safe"}">${cloudSession.admin?"Admin account":cloudSession.syncing?"Syncing…":cloudSession.error?"Cloud sync failed":cloudSession.lastSync?"Synced · "+cloudSession.lastSync.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"}):"Cloud connected"}</div></div>`:`<div class="cloudAvatar">☁</div><div class="grow"><b>Not signed in</b><div id="cloudStatusText" class="small">Sign in to save data permanently to Firebase.</div></div>`}</div><div class="cloudActions">${user?`${cloudSession.admin?`<button class="btn soft" data-act="adminDashboard">👑 Admin Dashboard</button>`:`<button class="btn soft" data-act="syncNow">☁️ Sync Now</button>`}<button class="btn outline" data-act="logout">Logout</button>`:`<button class="btn" data-act="studentLogin">Student Login</button><button class="btn outline" data-act="adminLogin">Admin Login</button>`}</div>${user?`<div class="cloudActions secondary"><button class="btn outline" data-act="forgotPassword">Forgot Password</button></div>`:`<p class="small">Student and Admin accounts use Firebase Email/Password authentication. Admin access requires an authorized Firebase admin role.</p>`}</div>`;
...
function adminDashboard(){
  ...
  if(!adminCache.students.length)return `<div class="card formCard"><div class="row" style="justify-content:space-between"><b>👑 Admin Dashboard</b><button class="btn soft" data-act="refreshAdmin">Refresh</button></div><p class="small">No student profiles have been synced to Firebase yet.</p></div>`;
...
function adminStudentPage(uid){
  const s=adminCache.students.find(x=>x.uid===uid), days=adminCache.days||[]; if(!s)return `<div class="card formCard"><p>Student not found.</p><button class="btn" data-act="backFromStudent">Back</button></div>`;
  const flat=[];
  days.sort((a,b)=>String(a.date).localeCompare(String(b.date))).forEach(d=>{
    const sessions=d.sessions||[]; const max=Math.max(sessions.length,Object.keys(d.records||{}).length,Object.keys(d.status||{}).length);
    for(let i=0;i<max;i++){
      ...
      flat.push({date:d.date,index:i,status,subject,scheduled:rec.scheduled||snap.scheduledSubject||subject,start:rec.start||snap.start||"",end:rec.end||snap.end||"",scheduledStart:rec.scheduledStart||snap.start||"",scheduledEnd:rec.scheduledEnd||snap.end||"",note:d.note||"",holiday:d.holiday,type:rec.type||classTypeForSnapshot(snap,rec,i,sessions)});
    }
    if(d.note&&!max)flat.push({date:d.date,index:"",status:"note",subject:"",scheduled:"",start:"",end:"",scheduledStart:"",scheduledEnd:"",note:d.note,holiday:d.holiday,type:"Note"});
  });
  const stats=adminCompute(days);let h=`<div class="card formCard adminHero"><div class="row" style="justify-content:space-between"><div><b>👤 ${esc(s.name||"Student")}</b><div class="small">Roll ${esc(s.rollNumber||"—")} · ${esc(s.email||"")}</div></div><button class="btn outline" data-act="backFromStudent">Back</button></div><div class="adminSummaryGrid"><div><b>${stats.present}</b><span>Present</span></div><div><b>${stats.absent}</b><span>Absent</span></div><div><b>${stats.pending}</b><span>Pending</span></div><div><b>${stats.total}</b><span>Total</span></div></div><div class="adminPercent">Overall attendance: <b>${stats.percent===null?'—':stats.percent.toFixed(1)+'%'}</b></div></div><div class="card formCard"><b>📚 Subject-wise attendance</b>${Object.entries(stats.subjects).sort((a,b)=>a[0].localeCompare(b[0])).map(([sub,x])=>`<div class="adminSubjectRow"><b>${esc(sub)}</b><span>${x.held?((x.present/x.held)*100).toFixed(1)+'%':'—'}</span><small>Present ${x.present} · Absent ${x.absent} · Pending ${x.pending} · Not Held ${x.notHeld} · Total ${x.total}</small></div>`).join('')}</div><div class="card formCard"><div class="row" style="justify-content:space-between"><b>📋 Detailed records</b><div class="cloudActions"><button class="btn soft" data-act="exportAdminCSV">Export CSV</button><button class="btn soft" data-act="exportAdminJSON">Full JSON</button><button class="btn outline" data-act="printAdminReport">Print / PDF</button></div></div><div class="adminTableWrap"><table class="adminTable"><thead><tr><th>Day</th><th>Date</th><th>Scheduled Time</th><th>Actual Time</th><th>Subject</th><th>Type</th><th>Status</th><th>Note</th></tr></thead><tbody>${flat.map(r=>`<tr><td>${r.date?esc(DAYS[parseKey(r.date).getDay()]):''}</td><td>${esc(r.date||'')}</td><td>${r.scheduledStart&&r.scheduledEnd?esc(time(r.scheduledStart)+'–'+time(r.scheduledEnd)):'—'}</td><td>${r.start&&r.end?esc(time(r.start)+'–'+time(r.end)):'—'}</td><td>${esc(r.subject||'')}</td><td>${esc(r.type||'')}</td><td>${esc(adminStatusLabel(r.status))}</td><td>${esc(r.note||'')}</td></tr>`).join('')}</tbody></table></div></div>`;return h;
}
...
function printAdminReport(){window.print()}
