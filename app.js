...
function applyTheme(){const ed=(state.settings&&state.settings.edition)||"earth-day";document.body.classList.toggle("nebula-glass",ed==="nebula-glass");document.body.classList.toggle("galactic-night",ed==="galactic-night");document.body.classList.toggle("black-hole",ed==="black-hole");document.body.classList.toggle("heart-nebula",ed==="heart-nebula");document.body.classList.toggle("roshni",ed==="roshni");document.body.setAttribute("data-edition",ed);
...
function attendanceAdvice(r){
  ...
  if(q>=1){
    ...
    return '<span class="adviceRed attendance-message-red" style="color:#dc3545!important">⚠ Need to attend all future classes; 100% can only be reached if no class was missed</span>';
  }
  if(r.pct>=req){
    let canMiss=Math.floor(r.attended/q-r.held+1e-9);if(canMiss<0)canMiss=0;
    if(canMiss===0)return '<span class="adviceGreen attendance-message-green" style="color:#198754!important">✓ Attendance is enough,</span><span class="adviceRed attendance-message-red" style="color:#dc3545!important"> ⚠ but you cannot miss the next class</span>';
    return '<span class="adviceGreen attendance-message-green" style="color:#198754!important">✓ Attendance is enough, you can miss '+canMiss+' more class'+(canMiss===1?'':'es')+' and still stay at or above '+req+'%</span>';
  }
  let needed=Math.ceil((q*r.held-r.attended)/(1-q));if(needed<0)needed=0;
  return '<span class="adviceRed attendance-message-red" style="color:#dc3545!important">⚠ Need to attend the next '+needed+' class'+(needed===1?'':'es')+' consecutively to reach '+req+'%</span>';
}
