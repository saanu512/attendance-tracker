...
function adminStudentPage(uid){
  ...
  const detailRows=grouped.map(g=>{
    ...
    return `<tr class="adminDateGroup"><th colspan="5">${esc(label)}</th></tr>${g.rows.map(r=>{
      ...
      return `<tr><td>${esc(scheduled)}</td><td>${esc(actual)}</td><td>${esc(r.subject||'—')}</td><td>${esc(r.type||'Regular')}</td><td>${esc(adminStatusLabel(r.status))}</td><td>${esc(r.note||'')}</td></tr>`;
    }).join('')}`;
...
function reportDetailHtml(grouped){
  return grouped.map(g=>`<section class="reportDay"><h3>${esc(g.label)}</h3><div class="reportTable"><div class="reportTableHead"><span>Scheduled Time</span><span>Actual Time</span><span>Subject</span><span>Type</span><span>Status</span><span>Note</span></div>${g.rows.map(r=>`<div class="reportTableRow"><span>${esc(r.scheduled)}</span><span>${esc(r.actual)}</span><span><b>${esc(r.subject)}</b></span><span>${esc(r.type)}</span><span>${esc(r.status)}</span><span>${esc(r.note||"—")}</span></div>`).join("")}</div></section>`).join("")||'<div class="small">No attendance records found.</div>';
}
